...
<%	
	...
	if(type==null){
		...
		sql="select * from gongji where title like '%"+content+"%' order by rootid desc, nodeid asc ;";
	}else if(type.equals("titleText")){
		sql="select * from gongji where title like '%"+content+"%' or content like '%"+content+"%' order by rootid desc, nodeid asc ;";
	}else{
		sql="select * from gongji where content like '%"+content+"%' order by rootid desc, nodeid asc ;";
	}
...
<tr><!--공지사항 리스트 출력부분-->
		...
		<td align=center><a href="gongji_view.jsp?id=<%=rset.getInt(1)%>&rootid=<%=rset.getInt(5)%>&nodeid=<%=rset.getInt(6)%>"><%=rset.getString(2)%></td>
